...
public class HttpPusher extends FormPusher {
    ...
    public HttpPusher(HttpQuery http, String url) {
        this(http, HttpAsyncQuery.TYPE_GET, url);
    }

    public HttpPusher(HttpQuery http, int method, String url) {
        this.http = http;
        this.url = url;
        this.method = method;
    }
    ...
    @Override
    protected void onPush(Form form, final FormCallback callback) {
        http.addParams(form);
